...
## Version removal or re-indexing requests

The [Version Immutability policy](POLICY.md#version-immutability) explains that we do not remove or re-index versions for routine reasons (CI double-builds, minor bugs, changed minds). Requests come in through the [Version Removal Request](https://github.com/opentofu/registry/issues/new?template=version-removal-request.yml) and [Version Re-indexing Request](https://github.com/opentofu/registry/issues/new?template=version-reindex-request.yml) issue templates.
...
2. If the request cites something in the exceptional category (malicious code, leaked secrets, compromised signing key, legal requirement, extreme disruption), verify the evidence provided (advisory links, CVEs, etc.) before acting.
