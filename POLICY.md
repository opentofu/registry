...
Once a provider or module version has been published and indexed, the OpenTofu Registry treats it as immutable. Consumers and automation rely on the assumption that a given version number always resolves to the same artifacts and checksums; re-indexing or removing a version on demand undermines that trust for everyone who depends on it, not just the requester.

For this reason, the OpenTofu Registry will **not** re-index or remove a version for reasons such as:
...
In all of these cases, the correct fix is for the provider or module author to release a new version in their own repository. Consumers who need the fix can then upgrade to it, while anyone already relying on the original version is not disrupted by it changing or disappearing out from under them.

The registry may remove or re-index a version on a case-by-case basis for exceptional circumstances, including but not limited to:
...
If you believe your situation qualifies, open a [Version Removal Request](https://github.com/opentofu/registry/issues/new?template=version-removal-request.yml) or [Version Re-indexing Request](https://github.com/opentofu/registry/issues/new?template=version-reindex-request.yml) issue and make your case, including any relevant evidence such as security advisories or CVEs.

We also encourage provider and module authors to enable [GitHub's immutable releases](https://docs.github.com/en/code-security/concepts/supply-chain-security/immutable-releases) on their repositories. This provides an upstream, technical guarantee that complements this policy, though the registry's own practice of not re-indexing or removing versions applies regardless of whether a given repository has this feature enabled.
